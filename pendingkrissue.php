...
<?php
    if(isset($_POST['btn-search']))
    {
        ...
        $selvrno = mysqli_query($conn,"SELECT *,DATE_FORMAT(orderdt, '%d-%m-%Y'),DATE_FORMAT(disdate, '%d-%m-%Y') FROM orderpro where orderno = $selorno and status = 'pending'") or die("could not search record");
...
?>
    ...
    <h2 id="title"> Pending Karigar Issue </h2>
    <?php
...
<?php
        $select = "SELECT *,DATE_FORMAT(orderdt, '%d-%m-%Y'),DATE_FORMAT(disdate, '%d-%m-%Y') FROM orderpro where status = 'pending'";
...
?>
    ...
    <div style="display: flex; justify-content: center;">
                <a href="pendingkrissue_print.php" class="btn btn-primary"> Print </a>
    </div>
